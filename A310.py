...
class Ui_MainWindow(object):
    ...
    def update_item_data(self, data):
        ...
        if length >= 3:
            ...
            cur = ''
            try:
                conn = pymssql.connect(server, user, password, "AIS20180101102412", charset='GBK')
                cur = conn.cursor()
                cur.execute(
                    "SELECT a.FNumber,a.FName,a.FModel,a.FErpClsID,b.FName,a.FItemID FROM t_ICItem as a join t_MeasureUnit as b on a.FUnitID=b.FItemID  where a.FNumber like '%" + str(
                ...
                if result:
                    for a in result:
                        ...
                        result1_sql = "SELECT a.FQty,b.fname,a.FBatchNo,a.fitemid FROM ICInventory as a join t_Stock as b on a.FStockID=b.fitemid where a.FQty<>0 AND a.FItemID='" + str(
                            a[
                                5]) + "' UNION ALL SELECT a.FQty,b.fname,a.FBatchNo,a.fitemid FROM POInventory as a join t_Stock as b on a.FStockID=b.fitemid where a.fqty>0 and a.FItemID='" + str(
                            a[5]) + "'"
                        ...
                        if result1:
                            for b in result1:
                                ...
                                self.tableWidget.setItem(dqhs, 0, QtWidgets.QTableWidgetItem(str(a[0])))
                                # 名称
                                self.tableWidget.setItem(dqhs, 1, QtWidgets.QTableWidgetItem(str(a[1])))
                                # 规格
                                self.tableWidget.setItem(dqhs, 2, QtWidgets.QTableWidgetItem(str(a[2])))
                                # 属性
                                if a[3] == 2:
                                ...
                                # 单位
                                self.tableWidget.setItem(dqhs, 4, QtWidgets.QTableWidgetItem(str(a[4])))
                                # 仓库
                                self.tableWidget.setItem(dqhs, 5, QtWidgets.QTableWidgetItem(str(b[1])))
                                # 批次
                                self.tableWidget.setItem(dqhs, 6, QtWidgets.QTableWidgetItem(str(b[2])))
                                # 数量
                                self.tableWidget.setItem(dqhs, 7, QtWidgets.QTableWidgetItem(str(round(b[0], 2))))
                        else:
                            dqhs = self.tableWidget.rowCount()
                            self.tableWidget.insertRow(dqhs)
                            self.tableWidget.setItem(dqhs, 0, QtWidgets.QTableWidgetItem(str(a[0])))
